...
/**
 * Load keys from archon-keymaster/nostr or fallback to nostr.env
 */
function loadKeys() {
  const nostrEnv = path.join(process.env.HOME, '.config/hex/nostr.env');
  
  // Try to load from nostr.env (already derived by archon-keymaster/nostr)
  ...
  // If archon-keymaster/nostr available, try to derive keys
...
/**
 * Encrypt a file using archon-keymaster/crypto
 */
function encryptFile(inputPath, outputPath, recipientAlias = null) {
  if (!skillAvailable('crypto')) {
    throw new Error('archon-keymaster/crypto not available');
  }
...
/**
 * Decrypt a file using archon-keymaster/crypto
 */
...
/**
 * Backup to Archon vault using archon-keymaster/backup
 */
function backupToVault(sourcePath, vaultName) {
  if (!skillAvailable('backup')) {
    throw new Error('archon-keymaster/backup not available');
  }
