 ...
 * 
 * Integrates with archon-* skills when available:
 * - archon-crypto: encrypt/decrypt files
 * - archon-backup: vault operations
 * - archon-nostr: key derivation
 * 
...
// Skill locations (relative to clawd)
const CLAWD_DIR = process.env.CLAWD_DIR || path.join(process.env.HOME, 'clawd');
const SKILLS = {
  crypto: path.join(CLAWD_DIR, 'skills/archon-crypto/scripts'),
  backup: path.join(CLAWD_DIR, 'skills/archon-backup/scripts'),
  nostr: path.join(CLAWD_DIR, 'skills/archon-nostr/scripts')
};
